...
class ArticlesWithWordsView(views.APIView):
    ...
    def get(self, request):
        ...
        practice_words = Word.objects.filter(
            language=language, rank__gte=1, rank__lte=100
        ).all()
        known_words = Word.objects.filter(
            language=language, rank__gte=1, rank__lte=500
        ).all()
        practice_words = [w.text for w in practice_words]
        known_words = [w.text for w in known_words]

        articles = self.getArticlesWithWords(
            language, practice_words, known_words, "2020-01-01", "2023-01-01", 100
        )
        ...
        lemmas = (
            ...
            .filter(language=language, text__in=lemmas_to_find)
            .order_by("rank")
            .all()
        )
...
class UserWordsViewSet(views.APIView):
    ...
    def get(self, request):
        ...
        lemmas = (
            lemmas.filter(language="hebrew", rank__gte=1, rank__lte=100)
            .order_by("rank")
            .all()
        )
